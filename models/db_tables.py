...
dt('task',
    Field('uuid','string',length=64, default=uuid.uuid4(),
          writable=False),
   Field('project_uuid', 'string', 
         requires=IS_IN_DB(db(db.project.close == False), 'project.uuid', 'project.name'), 
    ...
    Field('finish', 'datetime', default=request.now, requires=IS_DATE()),
    Field('closed', 'boolean', default=False,
